...
// The Mage-Flow content policy, VERBATIM from the reference
// (mage_flow/models/modules/mage_text.py: CONTENT_FILTER_SYSTEM /
// CONTENT_FILTER_EDIT_SYSTEM). These strings ARE the policy -- the classifier
// is a general Qwen3-VL, so every category, every "do NOT rationalize" clause
// and every worked example below is load-bearing on what it blocks.
//
// This translation unit is DATA. Do not re-wrap it to 80 columns, do not
// reflow the prose, do not "tidy" the examples: any edit here silently
// changes what the gate lets through. Byte-identical to the reference is the
// only property worth having, and tests/unit-tests/mage-screen.cc pins the
// pieces that decide a verdict.

namespace vpipe {
namespace genai {

const char* const kMageFilterSystem = R"MAGE(You are a strict content-policy classifier for a text-to-image system following Microsoft Responsible AI guidelines. Decide whether the user's text-to-image prompt requests content in any of these disallowed categories. Be CONSERVATIVE: implicit, suggestive, contextual, coded, and "documentary-framed" depictions ALL count.
...
→ {"violates": false, "categories": [], "reason": "Benign food photography."})MAGE";

const char* const kMageFilterEditSystem = R"MAGE(You are a strict content-policy classifier for an IMAGE-EDITING system following Microsoft Responsible AI guidelines. You are shown one or more SOURCE IMAGES and a text EDIT INSTRUCTION. Decide whether this edit request must be blocked. Be CONSERVATIVE.
...
→ {"violates": false, "categories": [], "reason": "Not a recognizable public figure; ordinary person, innocuous edit — allowed."})MAGE";
