// letterbox_planar_u8_to_u8_chw.metal -- bilinear letterbox resample,
// planar U8 [3, in_h, in_w] -> planar U8 [3, out_h, out_w], aspect-
// preserving scale with 114 grey padding outside the letterbox region.
// Same geometry as letterbox_planar_u8_to_f32_chw, but the output stays
// 8-bit (no /255 normalisation) so it can feed a vision encoder that
    ...
    uint2               gid       [[thread_position_in_grid]])
{
  ...
  if (out) {
    dst[              dst_idx] = 114;
    dst[plane_dst   + dst_idx] = 114;
    dst[2*plane_dst + dst_idx] = 114;
    return;
  }
