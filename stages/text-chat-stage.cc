...
const PortSpec kIports[] = {
  {.name = "user", .doc = "FlexData string: the user's turn text",
...
TextChatStage::process(RuntimeContext& ctx)
{
  ...
  vector<int32_t> ids;
  tpl->render_user_turn(user_text, /*is_first_turn=*/!_seeded, &ids);

  using clock = std::chrono::steady_clock;
  const auto t_prefill_start = clock::now();
  int32_t next = _lm->prefill(_chat_ctx, ids);
