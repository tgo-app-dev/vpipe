...
- **[`minimax-h3-extend-part1.vpipeline`](pipelines/minimax-h3-extend-part1.vpipeline)**
  ...
  续接，每一段都从上一部分的最后 2.33 s 接着往下走（见
  [更长的片段](#longer-clips--one-story-in-four-parts)）。需要两个模式，而且每一
  部分都会应用它所属模式自己的 Turbo 适配器。
- **[`minimax-h3-extend-concat.vpipeline`](pipelines/minimax-h3-extend-concat.vpipeline)**
  ——通过 concat demuxer 把这四部分拼接成一个 33 秒的文件，不需要模型，也不需要手写
  ffmpeg。
...
按行数的平方计算。要得到 **33 秒**的片段，跑四次普通的 10 秒生成是更便宜的做法。
第一次之后的每一次都会把**前一次的尾段**作为要续接的片段交给 Ref2VA，于是故事由
画面和声音向前承接，而不是只靠提示词：

- **[`minimax-h3-extend-part1.vpipeline`](pipelines/minimax-h3-extend-part1.vpipeline)**
  ——在 **FL2VA** 模型文件上由文本生成视频与音频，243 帧（10.125 s）。它写出
...
- **[`…-part2`](pipelines/minimax-h3-extend-part2.vpipeline)** /
  ...
  **[`…-part4`](pipelines/minimax-h3-extend-part4.vpipeline)**
  ——**Ref2VA**，每一部分都以前一部分最后 2.33 s 的画面和声音为条件。各 243 帧。
...
**每个模式各有自己的 Turbo 适配器，而这不是流水线图能替你隐藏的细节。**适配器是
针对单一任务蒸馏出来的，所以第 1 部分应用 **FL2VA** 的适配器，第 2 到 4 部分应用
**Ref2VA** 的适配器；模型目录中的 parent 链接负责把两者各自挡在对方的流水线图之外
（见[哪些 Turbo 适配器可用](#which-turbo-adapters-work)）。随附的配置是：

| | 适配器 | 步数 | shift 值 |
|---|---|---|---|
| 第 1 部分 | `larryvrh/MiniMax-H3-Turbo-Lora-v4-600-ema` | 8 | 12 / 3 |
| 第 2–4 部分 | `lightx2v/Minimax-h3-Turbo-ref2va-8step-768p` | 8 | **6** / 3 |

这里只有 Ref2VA 的适配器需要 **`video_shift: 6.0`**，它的流水线图已经设好了。它在
**运行时**应用而不是融合进权重：Turbo 的权重增量是 2–4e-4，而这些权重本身的 bf16
步进约为 4e-3，所以融合会把其中大部分四舍五入掉。去掉那两个 `lora` 配置项，各
部分就会在相同的步数下用基础模型文件运行。

> **Turbo 适配器是一次蒸馏，它有其局限。**换来更少的步数，代价是牺牲一部分基础
> 模型对构图的理解：某一部分可能返回主体位置很糟的画面、一只画错的手，或者干脆
> 漏掉提示词要求的某个节拍，而未蒸馏的模型在更高步数下本来能保住它们。它们本身
> 也在**不断变好**——上面两条都已经被重新蒸馏过不止一次，新版本往往
> 修正了旧版本做错的地方，所以在固定使用某一份副本之前先确认发布者。
>
> 当某一部分返回的构图不理想时，在重写提示词之前有两件事值得一试。**换一个
> `seed`** 更便宜，而且常常就够了，因为适配器丢掉的是一次采样所包含的结构，而不是
> 提示词的含义。如果还不行，就**不加适配器、用 16–20 步**跑那一部分：那才是基础
> 模型自身的质量，也是评判适配器的标准。去掉它的 `lora` 和 `lora_scale` 配置项并
> 提高 `steps`；流水线图的其他部分都不变，它之后的各部分仍然只读它的尾段。
...
你需要**两个**模式都准备好（步骤 1 和
[准备 Ref2VA 模型文件](#preparing-the-ref2va-checkpoint)）。按顺序运行，并且在同一个
工作目录下——每一部分都按相对名称打开前一部分的 `.mp4`：
...
**切帧用的是 `temporal-slice`，不是 seek。**`load-video` 的 `start_s` 会落在所请求
时间点或其之前的关键帧上，在一个 10 秒的文件里那可能就是第一帧。负的 `start` 会
正好保留直到流结束的最后 56 个解码帧，所以切割精确到帧，代价只是把一个短文件解码
一遍。音频包是独立解码的，所以 `load-audio` 可以按时间取它的窗口。那个窗口是从
`243/24 − 56/24 = 7.7917 s` 开始，与这些帧覆盖的是同样的 2.33 s。

**`attach_audio: [2]`** 把那条声轨折叠到它前面的片段上，使模型读到的是一个
`<Video 1>` 及其 `<Audio 1>`，而不是两个互不相关的参考（见
[属于某个片段的音频](#audio-that-belongs-to-a-clip)）。

**为什么是 56 帧而不是 48。**24 fps 下两秒是 48 帧，但编码器会**从开头起**把参考
片段向下取整到整数个 `17n + 5` 的分块数。给它 48 帧，它保留 39 帧，而它丢掉的那 9
帧正是最后那几帧——恰恰是续接本该接上的那一刻。56 是 `17 × 3 + 5`，所以什么都不会
被丢掉。编码器打印的那一行可以确认：8,670 个参考视频行就是 17 个潜变量帧、每帧
510 个单元。MiniMax 记载的最短参考片段是 **2 s**，2.33 s 满足这个要求。

**`reference_video_short_edge: 0`** 让引导片段保持在它自己的 960 × 544 画布上，也
就是输出本身的尺寸，而不是把它放大到 1344 × 768。这样参考就保持在 8,670 行，而不
...
部分以六秒静止的中近景收尾，其间除了烛焰什么都不动；第 2 部分从那个取景开始，并在
鸟醒来之前保持两秒；第 2 部分反过来又结束在鸟静止于窗台上，依此沿链而下。引导片段
总是从这些静止段之内切出来的，所以每一处接缝都落在画面静止的地方。从一开始就照这
个思路规划故事：值得生成的节拍是那些**接缝之间**的部分。

**逐字沿用主体定义。**第 2 到 4 部分用完全相同的词句描述那个女人、那只鸟和那个
房间，因为片段展示了它们而文本又命名了它们——措辞上的任何漂移，都是模型可以随意
施加到画面上的漂移。有些漂移还是会出现；见下文。

**Ref2VA 是在片段之后续接；它不会重放片段。实测**：第 2 部分的第一帧最接近第 1
部分的**最后**一帧（31.5 dB PSNR），远比接近引导窗口的第一帧（26.3 dB）。这是模型
从任务类型中学到的续接，而不是一个被钉住的帧。
...
每一部分最后的 2.33 s 和下一部分开头的静止段是同一刻，所以剪切点可以落在其中的
任何位置。

**在 vpipe 内**，那就是
[`minimax-h3-extend-concat.vpipeline`](pipelines/minimax-h3-extend-concat.vpipeline)：
一个 `load-video` 通过 **concat 解复用器（demuxer）**读取一个**列表**而不是一个
文件，每一部分带一个 `outpoint`，在它的接缝处把它裁断。
...
`video_bitrate` 重新编码一次，两条流在相同的位置被切开。有两个配置项使它成立：
`format: "concat"`（否则一个列表文件会被探测成 ANSI 字符画，因为一个文本文件在
探测器看来就是那样）以及 `options: {"safe": "0"}`，解复用器要先有它才愿意跟随
绝对路径。

**手工拼接**时，如果接缝处想要交叉淡化而不是硬切——流水线里没有做这件事的
stage——同样这四个文件可以各取到它的引导片段开始之处，并在五帧之内淡化：
...
每个 `offset` 都是下一部分的静止段在目前已构建的时间线上开始的位置：7.792，然后是
7.792 × 2，再是 7.792 × 3。结果是 **33.5 s**——四部分的 40.5 s 减去三段 2.33 s 的
重叠。由于每一处接缝都落在一个静止镜头之内，硬切也可行：**实测**，每一部分的最后
一帧与下一部分的第一帧分别落在 **31.4、29.8 和 32.7 dB** PSNR。

<a id="what-the-parts-cost"></a>
#### 各部分的开销

四个流水线图都随附**[`sol_attn`](#faster-attention--sol-attn-routing) 开启**，这
也是让这么长的片段变得负担得起的主要原因。**实测**于 24 GB 的 M5 Pro，8-bit，
960 × 544，243 帧，8 步，并在运行时应用上面那两个 Turbo 适配器：

| | 打包行数 | 去噪 | 总耗时 | 峰值内存 |
|---|---|---|---|---|
| 第 1 部分（文本） | 38,072 | 9 min 3 s | 10 min 52 s | 18.1 GB |
| 第 2 部分 | 49,152 | 16 min 32 s | 20 min 29 s | 17.0 GB |
| 第 3 部分 | 49,111 | 16 min 28 s | 20 min 24 s | 16.9 GB |
| 第 4 部分 | 49,102 | 16 min 35 s | 20 min 30 s | 17.9 GB |
| 拼接 | — | — | **2.1 s** | — |

整个 33.5 秒的片段需要 **1 h 12 min** 的生成时间，而一次续接无论位于链条的多远处，
开销都和它的同伴一样——每一个都只读取正好 2.33 s 的视频，不管它之前有什么。

**这里的适配器买到的是质量，不是时间。**两者都是 8 步蒸馏，而这些运行本来就用了 8
步，所以 LoRA 的代价就是它自己的计算量——在总耗时上约 3%，对比同样四部分不用它们时
的 1 h 10 min。它们的价值体现在**更低**的步数上：[更少的步数](#fewer-steps--the-turbo-lora)
一节表格中的 4 步条目把这个时间减半，而 Ref2VA 那一行就有一个。

一次续接比它所接续的那次文本生成开销更大：引导片段增加了 8,670 个视频行和 190 个
音频行，而条件部分增长到约 2,750 行，因为视觉塔对该片段的解读也是提示词的一部分。
这是 1.29× 的行数换来 1.87× 的总耗时——注意力的开销按行数的平方计算，而参考编码在
第一步之前还要增加大约两分钟。

Sol 在续接上节省得比在文本生成上少：它在第 1 部分保持了 **24%** 的 key 分块为精确
计算，在第 2 部分则是 **41%**，因为参考行会被各处都关注到。
...
- **改变某一部分的长度，下一部分的音频窗口就会移动。**`start_s` 是
  `previous_part_frames / 24 − 56 / 24`。只要切片仍停在 56，`duration_s` 就保持
  2.3333。
- **更长的引导片段是 `17n + 5` 帧**：73（3.04 s）或 90（3.75 s）。改
  `temporal-slice` 的 `start`，并随之改音频窗口。更长的引导片段承载更多运动，每多
  一个潜变量帧约需 510 行。
- **要预期身份沿这条链漂移**，并针对它来写。每一部分只看到它之前的 2.33 s，所以
...
- **更换 Turbo 适配器会连带改变步数和 shift 值。**每一个都是按某个配方蒸馏出来
  的；上面的表格是随附那一对所需要的，而
